...
export async function initDb(): Promise<void> {
  await pool.query(`
    ...
    -- An access request is either for SENSITIVE (unmasked) log access, or for
    -- UPLOAD access (a viewer asking to become an analyst so they can upload).
    ALTER TABLE access_requests ADD COLUMN IF NOT EXISTS type TEXT NOT NULL DEFAULT 'sensitive'; -- sensitive | upload
    ...
    -- RECYCLE BIN: every deletable category gets a soft-delete pair of
    -- columns instead of being removed outright. A row with deleted_at set
    -- is "in the recycle bin"; nothing is actually gone until it is
    -- permanently deleted (or restored) from there.
    ALTER TABLE logs        ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ;
    ALTER TABLE logs        ADD COLUMN IF NOT EXISTS deleted_by UUID REFERENCES users(id);
  ...
  if (email && password) {
    ...
    await pool.query(`UPDATE users SET disabled = FALSE, deleted_at = NULL WHERE email = $1`, [email]);
  ...
  // Seed sensible defaults for the admin config screens, so the pages
  ...
  // Seed a few common ingestion sources if none exist yet.
