...
// changed there. Nothing in this panel may touch that account.
...
const roleSchema = z.object({
  // 'admin' is not allowed here on purpose: there is only one admin.
...
router.delete('/users/:id', async (req, res) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM user_log_access WHERE user_id = $1`, [req.params.id]);
    await client.query(`DELETE FROM access_requests WHERE user_id = $1`, [req.params.id]);
    await client.query(`UPDATE logs SET uploaded_by = NULL WHERE uploaded_by = $1`, [req.params.id]);
    await client.query(`UPDATE audit_log SET user_id = NULL WHERE user_id = $1`, [req.params.id]);
    ...
    res.status(409).json({ error: 'Could not delete this user. Disable the account instead.' });
  } finally {
    client.release();
  }
});
