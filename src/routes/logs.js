...
const { maskNormalizedEvent } = require('../utils/masking');
...
router.post('/upload', async (req, res) => {
  ...
  const masked = maskNormalizedEvent(normalized);
...
router.get('/search', async (req, res) => {
  ...
  res.json(rows.map((log) => ({ eventId: log.id, ...log.masked_json })));
});
...
router.get('/:eventId', async (req, res) => {
  ...
  res.json({ eventId: log.id, ...log.masked_json });
});
...
router.post('/:eventId/unlock', async (req, res) => {
  ...
  res.json({ eventId: log.id, ...decrypted, masked: false });
});
