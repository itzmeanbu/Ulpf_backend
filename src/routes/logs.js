...
router.get('/search', async (req, res) => {
  ...
  res.json(rows.map((log) => ({ ...log.masked_json, eventId: log.id })));
});
...
router.get('/stats', async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      recentEvents: recentRows.map((log) => ({ ...log.masked_json, eventId: log.id })),
    });
  } catch (err) {
    next(err);
  }
...
router.get('/:eventId', async (req, res) => {
  ...
  res.json({ ...log.masked_json, eventId: log.id });
});
...
router.post('/:eventId/unlock', async (req, res) => {
  ...
  res.json({ ...decrypted, eventId: log.id, masked: false });
});
